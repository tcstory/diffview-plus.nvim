...
describe("diffview.runtime.process", function()
  ...
  it("wait() returns a result with code 0 for a successful command", async_test(function()
    local p = Process.start({ "echo", "hello" })
    local result = p:wait()
  ...
  it("wait() captures exit code for a failing command", async_test(function()
  ...
  it("succeeded() returns true only for exit code 0 and not killed", async_test(function()
    local p = Process.start({ "true" })
    p:wait()
    assert.is_true(p:succeeded())
  end))

  it("kill() sets the killed flag and terminates the process", async_test(function()
  ...
  it("is_done() returns false before wait() and true after", async_test(function()
  ...
  it("Process.start() merges GIT_OPTIONAL_LOCKS=0 into environment", async_test(function()
  ...
  it("extra env variables are passed through", async_test(function()
    local p = Process.start(
  ...
  it("stdin option passes data to the process", async_test(function()
    local p = Process.start(
      { "cat" },
      { stdin = "hello stdin\n" }
    )
