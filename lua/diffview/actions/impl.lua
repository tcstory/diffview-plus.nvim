...
function M._is_applicable(fn, view)
  ...
  if t == "merge_only" then
    ...
    return view ~= nil
      and (
        (view --[[@as DiffView]]).merge_ctx ~= nil or (view --[[@as any]]).merge_session ~= nil
      )
  end
...
function M.cycle_layout()
  ...
  for _, entry in ipairs(files) do
    ...
    if view.resolve_history_layout then
      target = (view --[[@as FileHistoryView ]]):resolve_history_layout(target)
    end
...
function M.set_layout(layout_name)
  return function()
    ...
    if view.resolve_history_layout then
      target_layout = (view --[[@as FileHistoryView ]]):resolve_history_layout(target_layout)
    end
